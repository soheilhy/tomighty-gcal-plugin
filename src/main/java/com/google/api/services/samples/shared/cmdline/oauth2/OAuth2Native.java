...
public class OAuth2Native {
  ...
  public static Credential authorize(HttpTransport transport, JsonFactory jsonFactory,
      VerificationCodeReceiver receiver, Iterable<String> scopes, CredentialStore store) throws Exception {
    try {
      String redirectUri = receiver.getRedirectUri();
      GoogleClientSecrets clientSecrets = loadClientSecrets(jsonFactory);
      // redirect to an authorization page
      GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(
          transport, jsonFactory, clientSecrets, scopes).setCredentialStore(store).build();
      Credential cred = flow.loadCredential(""); 
      if (cred != null) {
          return cred;
      }
      ...
      return flow.createAndStoreCredential(response, "");
    } finally {
      receiver.stop();
    }
  }
